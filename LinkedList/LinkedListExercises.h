...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // Case 1: Empty list or insertion at the beginning
  if (!head_ || newData < head_->data) {
    newNode->next = head_;
    if (head_) 
      head_->prev = newNode; // Update previous pointer of the current head if it exists
      head_ = newNode;
    if (!tail_) // Update tail_ if the list was empty
      tail_ = newNode;
    size_++;
    return;
  }
