...
// NOT templated, such as treeFactory,ensure that the definition is
...
static void treeFactory(GenericTree<int>& tree) {
  ...
  node15->parentPtr = node4;
  node8->parentPtr = node4;
  auto node16 = node8->addChild(16);
  auto node23 = node8->addChild(23);
  node23->parentPtr = node8;
  node16->parentPtr = node8;
  node16->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!nodesToExplore.empty()) {
    auto curNode = nodesToExplore.front();
    nodesToExplore.pop();
    if (curNode != nullptr) {
      results.push_back(curNode->data);
    }
    for (auto childNode : curNode->childrenPtrs) {
